...
class Calculator:
    def __init__(self, master):
        ...
        top_frame = tk.Frame(master, width=650, height=50, bd=4, relief='flat', bg='black')
        top_frame.pack(side=tk.TOP)
        # set frame showing all buttons
        bottom_frame = tk.Frame(master, width=650, height=490, bd=4, relief='flat', bg='black')
        bottom_frame.place(x=0,y=87)
        # name of calculator
        my_item = tk.Label(top_frame, text="Scientific Calculator",
                           font=('arial', 30), fg='black', width=26, bg='#00FFFF')
        my_item.pack()
        # entry interface for inputs
        self.txt_display = tk.Entry(top_frame, font=('arial', 20), relief='flat',
                               bg='white', fg='black', textvariable=self.text_input, width=60, bd=3, justify='right')
        ...
        self.btn_sqrt.grid(row=4, column=4)
        # row 1
        ...
        self.btn_0.configure(activebackground="#4d4d4d", bg='#4d4d4d', width=2, bd=5)
        ...
        self.status=tk.Label(master,text='History:-',relief='sunken',height=2,anchor='w',font='verdana 11 bold')
        self.status.pack(side='bottom',fill='x')
        self.status_path=tk.Label(master,text='Developer:-Karan Kumawat|Ankit Kumawat|Dipendu Mondal',font='verdana 8 bold')
        self.status_path.pack(side='bottom',fill='x')
    ...
    def btn_equal(self):
        casee=['float division by zero','division by zero']
        content = self.txt_display.get()
        try:
            self.sum_up = str(eval(content))
        ...
        history.append(content+'='+self.expression)
        history.reverse()
        self.status.configure(text='History:- '+" | ".join(history[:4]),font='verdana 7 bold')
